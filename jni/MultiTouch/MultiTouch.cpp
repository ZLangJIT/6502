...
void MultiTouch::addTouch(const MultiTouch::TouchData & touchData) {
    if (debug) os_log_info("adding touch with identity: %ld", touchData.identity);
    bool found = false;
    for (long i = 0; i < maxSupportedTouches; i++) {
        ...
        if (touchContainer.used && touchContainer.touch.state == NONE) {
            if (throw_on_error) {
                os_log_fatal("touch cannot be active with a state of NONE");
            } else {
                os_log_error("touch cannot be active with a state of NONE, cancelling touch");
                cancelTouch();
            }
        }
    }
    if (!found) {
        if (throw_on_error) {
            os_log_fatal(
                    "the maximum number of supported touches of %ld has been reached.\n",
                    "please call setMaxSupportedTouches(long)", maxSupportedTouches
            );
        } else {
            os_log_error(
                    "the maximum number of supported touches of %ld has been reached.\n",
                    "please call setMaxSupportedTouches(long), cancelling touch", maxSupportedTouches
            );
            cancelTouch();
        }
    }
...
void MultiTouch::moveTouch(const MultiTouch::TouchData & touchData) {
    if (debug) os_log_info("moving touch with identity: %ld", touchData.identity);
    bool found = false;
    for (long i = 0; i < maxSupportedTouches; i++) {
        TouchContainer & touchContainer = data[i];
        tryPurgeTouch(touchContainer);
    ...
    if (!found) {
        if (throw_on_error) {
            os_log_fatal("cannot move a touch that has not been registered");
        } else {
            os_log_error("cannot move a touch that has not been registered, cancelling touch");
            cancelTouch();
        }
    }
}
...
void MultiTouch::removeTouch(const MultiTouch::TouchData & touchData) {
    if (debug) os_log_info("removing touch with identity: %ld", touchData.identity);
    bool found = false;
    for (long i = 0; i < maxSupportedTouches; i++) {
        TouchContainer & touchContainer = data[i];
        tryPurgeTouch(touchContainer);
    ...
    if (!found) {
        if (throw_on_error) {
            os_log_fatal("cannot remove a touch that has not been registered");
        } else {
            os_log_error("cannot remove a touch that has not been registered, cancelling touch");
            cancelTouch();
        }
    }
}
...
void MultiTouch::cancelTouch(const MultiTouch::TouchData & touchData) {
    if (debug) os_log_info("cancelling touch");
    bool found = false;
    for (long i = 0; i < maxSupportedTouches; i++) {
        TouchContainer & touchContainer = data[i];
        if (touchContainer.used && touchContainer.touch.state == NONE) {
    ...
    if (!found) {
        ...
        if (maxSupportedTouches <= 0) {
            os_log_error("the maximum number of supported touches must be greater than zero");
        } else {
