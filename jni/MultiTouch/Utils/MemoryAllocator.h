...
inline int os_log_info(const char* format, ... ) {
#ifdef __ANDROID__
    va_list args2;
...
inline int os_log_error(const char* format, ... ) {
#ifdef __ANDROID__
    va_list args2;
    va_start(args2, format);
...
inline void os_log_fatal(const char* format, ... ) {
    // set color to red
    fprintf(stderr, "\033[0;31m");
    va_list args;
    va_start(args, format);
    ...
    va_start(args2, format);
    //__android_log_vprint(ANDROID_LOG_ERROR, "GLIS", format, args2);
    __android_log_vprint(ANDROID_LOG_FATAL, "GLIS", format, args2);
    va_end(args2);
#endif
    abort();
}
...
namespace MemoryAllocator {
    ...
    public:
        ...
        typename std::enable_if<std::is_pointer<TAG>::value, T&>::type operator[](size_t index) {
            if (capacity <= index) {
                os_log_fatal("index out of range: index: %zu, capacity: %zu", index, capacity);
            }
            return data[index];
        }
